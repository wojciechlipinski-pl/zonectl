...
def test_external_migration_cli_is_read_only(monkeypatch, capsys) -> None:
    artifact = RpzMigrationArtifact(
        "updater", Path("/usr/local/sbin/update-cert-rpz.sh"), True,
        0, 0, "-rwxr-x---", "a" * 64,
    )
    plan = RpzExternalMigrationPlan(
        "READY", "cert-rpz.local", "external.timer", "external.service",
        True, True, (artifact,), Path("/managed/updater"),
        Path("/managed/service"), Path("/managed/timer"), Path("/backup"), (),
        ("wykonaj backup",), "Można przygotować dry-run.",
    )
    monkeypatch.setattr(cli.ToolkitConfig, "load", lambda self: _EmptyConfig())
    monkeypatch.setattr(
        cli.RpzExternalMigrationPlanner, "plan", lambda self: plan
    )
