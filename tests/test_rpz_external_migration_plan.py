...
def _external(zone_file: Path, updater: Path) -> RpzEnvironment:
    return RpzEnvironment(
        "cert-rpz.local", str(zone_file), "EXTERNAL", "ACTIVE", 10, 600,
        "123", 42, True, "external.timer", True, True, "external.service",
        "success", "now", "later", str(updater), (),
    )
...
def test_missing_external_artifact_blocks_migration(monkeypatch, tmp_path: Path) -> None:
    zone = tmp_path / "zone"
    ...
    runner = lambda command, timeout: CommandResult(1, "", "")
