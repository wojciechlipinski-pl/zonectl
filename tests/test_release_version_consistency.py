...
def test_release_version_is_consistent_across_public_artifacts() -> None:
    ...
    assert f"## {version} - 2026-09-07" in (ROOT / "CHANGELOG.md").read_text(
        encoding="utf-8"
    )
    assert f"**{version} —" in (ROOT / "README.md").read_text(encoding="utf-8")
    assert f"**{version} —" in (ROOT / "README.pl.md").read_text(encoding="utf-8")
    assert f'"ZoneCTL {version}"' in (ROOT / "debian/zctl.1").read_text(
        encoding="utf-8"
    )
