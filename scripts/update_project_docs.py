...
def scan(root):
    ...
    for p in sorted(src.rglob("*.py")):
        ...
        try:
            ...
            modules.append((p.relative_to(root), f"Błąd składni: {e}", [], [], todos))
            continue
        imports = []
        defs = []
        for n in tree.body:
            if isinstance(n, ast.Import):
                ...
                if isinstance(n, ast.ClassDef):
                    methods = [
                        (m.name, m.lineno, ast.get_docstring(m) or "")
                        for m in n.body
        ...
        modules.append(
            (p.relative_to(root), ast.get_docstring(tree) or "", imports, defs, todos)
        )
    return modules
...
def main():
    ...
    if docs.exists():
        ...
        print("Kopia:", backup)
    for d in [docs, docs / "api", docs / "diagrams", docs / "images"]:
        d.mkdir(parents=True, exist_ok=True)
    name, version = meta(root)
    modules = scan(root)
    now = datetime.now().astimezone().isoformat(timespec="seconds")
    branch = git(root, ["branch", "--show-current"], "(brak)")
    head = git(root, ["rev-parse", "--short", "HEAD"], "(brak)")
    last = git(
        root,
        ["log", "-1", "--date=iso", "--pretty=format:%h | %ad | %an | %s"],
        "(brak)",
    )
    status = git(root, ["status", "--short"], "czyste drzewo robocze")
    log = git(
        root,
        ["log", "--all", "--date=short", "--pretty=format:%ad\t%h\t%s", "-100"],
        "brak historii Git",
    )
    ...
    classes = sum(
        1 for _, _, _, ds, _ in modules for d in ds if d[0].startswith("class ")
    )
    funcs = sum(
        1
        for _, _, _, ds, _ in modules
        for d in ds
        if d[0].startswith(("def ", "async def "))
    )
...
""",
    ...
    for path, doc, imports, defs, t in modules:
        ref += f"## `{path}`\n\n{first(doc)}\n\n"
        if not defs:
            ref += "Brak klas i funkcji na poziomie modułu.\n\n"
    ...
    if not todos:
        todo += "Nie znaleziono znaczników.\n"
