...
class BindZoneSecondaryPlan:
    ...
    def transaction_plan(self) -> BindSecondaryPlan:
        return BindSecondaryPlan(
            name=f"zone-{self.zone}",
            kind="zone-assignment",
            source=self.source,
            old_addresses=self.old_pairs,
            new_addresses=self.new_pairs,
            roles=("notify", "transfer"),
            zones=(self.zone,),
...
class BindZoneSecondaryPlanner:
    ...
    def available_pairs(self) -> tuple[SecondaryPairReport, ...]:
        ...
        return tuple(
            pair
            for pair in BindSecondaryReporter().build(inventory).pairs
            if pair.status == "PASS"
        )

    def plan(
        self, zone_name: str, pair_names: list[str] | tuple[str, ...]
    ) -> BindZoneSecondaryPlan:
        ...
        if "rpz" in wanted or "rpz" in {
            part.casefold() for part in zone.config_file.parts
        }:
        ...
        for raw in pair_names:
            key = raw.casefold()
            if key not in pairs:
                raise BindZoneSecondaryError(
                    f"Nieznana lub niepełna para secondary: {raw}"
                )
            if key in selected:
        ...
        spans = [
            s
            for s in ManagedZoneMigrationPlanner._zone_spans(original, zone.config_file)
            if s.name.casefold() == wanted
        ]
        if len(spans) != 1:
            raise BindZoneSecondaryError(
                "Nie można jednoznacznie wydzielić deklaracji strefy"
            )
        span = spans[0]
        block = span.text
        current = tuple(
            sorted(
                ...
                if any(
                    group in self._directive_values(block, "also-notify")
                    for group in pair.notify_groups
                )
                and any(
                    group in self._directive_values(block, "allow-transfer")
                    for group in pair.transfer_groups
                )
            )
        )
        ...
        operational_addresses = tuple(
            dict.fromkeys(
                address for key in selected for address in pairs[key].notify_addresses
            )
        )
        candidate_block = self._set_directive(block, "also-notify", notify)
        candidate_block = self._set_directive(
            candidate_block, "allow-transfer", transfer
        )
        candidate = (
            original[: span.start]
            + candidate_block
            + original[span.start + len(block) :]
        )
        diff = "".join(
            difflib.unified_diff(
                ...
                tofile=f"{zone.config_file} (kandydat secondary strefy)",
            )
        )
        ok, message = BindSecondaryPlanner(self.root_config)._validate_candidate(
            zone.config_file, candidate
        )
        ...
        removed = tuple(
            item for item in current if item.casefold() not in selected_keys
        )
        ...
        impact = BindAccessImpactReport(
            ...
            candidate_entries=tuple(selected),
            added_entries=added,
        ...
        return BindZoneSecondaryPlan(
            zone=wanted,
            source=zone.config_file,
            old_pairs=current,
            new_pairs=tuple(selected),
    ...
    def _directive_values(cls, block: str, name: str) -> tuple[str, ...]:
        ...
        match = next(
            (
                m
                for m in cls._directive.finditer(masked)
                if m.group("name").casefold() == name
            ),
            None,
        )
        if match is None:
        ...
        return tuple(
            value.strip()
            for value in block[opening + 1 : closing].split(";")
            if value.strip()
        )
